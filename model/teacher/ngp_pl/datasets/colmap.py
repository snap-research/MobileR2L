...
class ColmapDataset(BaseDataset):
    ...
    def read_meta(self, split, **kwargs):
        ...
        if self.ff:
            self.centered_poses, _, self.avg_poses = center_poses(poses, pts3d) # type: ignore
        else: # 360
            self.centered_poses, _ = center_poses_mipnerf(poses)
        
        self.scale = np.linalg.norm(self.centered_poses[..., 3], axis=-1).min() 
        self.centered_poses[..., 3] /= self.scale

        self.mean_radius = np.linalg.norm(self.centered_poses[:, :3, 3], axis=-1).mean(0)
        self.min_radius =  np.linalg.norm(self.centered_poses[:, :3, 3], axis=-1).min()
        self.max_radius =  np.linalg.norm(self.centered_poses[:, :3, 3], axis=-1).max()
        self.min_theta, self.max_theta, self.min_phi, self.max_phi = cart_to_polar(self.centered_poses)
        print("min max theta: ", self.min_theta * 180 / np.pi, self.max_theta * 180 / np.pi)
        print("min max phi: ", self.min_phi * 180 / np.pi, self.max_phi * 180 / np.pi)
        self.rays = []
        if split == 'test_traj': # use precomputed test poses
            self.poses_traj = create_spheric_poses(1.2, self.centered_poses[:, 1, 3].mean())
            self.poses_traj = torch.FloatTensor(self.poses_traj)
            return
        ...
        if 'HDR-NeRF' in self.root_dir: # HDR-NeRF data
            if 'syndata' in self.root_dir: # synthetic
                ...
                if split=='train':
                    img_paths = sorted(glob.glob(os.path.join(self.root_dir,
                                                            f'train/*[024].png')))
                    self.poses = np.repeat(self.centered_poses[-18:], 3, 0)
                elif split=='test':
                    img_paths = sorted(glob.glob(os.path.join(self.root_dir,
                                                            f'test/*[13].png')))
                    self.poses = np.repeat(self.centered_poses[:17], 2, 0)
                else:
                ...
                if split=='train':
                    ...
                    self.poses = np.tile(self.centered_poses[::2], (3, 1, 1))
                elif split=='test':
                    img_paths = sorted(glob.glob(os.path.join(self.root_dir,
                                                    f'input_images/*1.jpg')))[1::2]
                    img_paths+= sorted(glob.glob(os.path.join(self.root_dir,
                                                    f'input_images/*3.jpg')))[1::2]
                    self.poses = np.tile(self.centered_poses[1::2], (2, 1, 1))
                else:
                    raise ValueError(f"split {split} is invalid for HDR-NeRF!")
        else:
            ...
            skip = 8
            if split=='train':
                img_paths = [x for i, x in enumerate(img_paths) if i%skip!=0]
                self.poses = np.array([x for i, x in enumerate(self.centered_poses) if i%skip!=0])
                self.test_poses = np.array([x for i, x in enumerate(self.centered_poses) if i%skip==0])
        ...
        for img_path in tqdm(img_paths):
            ...
            img = read_image(img_path, self.img_wh, blend_a=1)
