    ...
    def read_meta(self, split):
        ...
        for frame in tqdm(frames):
            ...
            if 'Jrender_Dataset' in self.root_dir:
                ...
                pose_radius_scale = 1.5
            print(np.linalg.norm(c2w[:, 3]))
            if not hasattr(self, 'scale'):
                self.scale = np.linalg.norm(c2w[:, 3])/pose_radius_scale
            ...
            try:
                ...
                img = read_image(img_path, self.img_wh, blend_a=True)
                self.rays += [img]
            except:
                pass

        if len(self.rays) > 0:
            self.rays = torch.FloatTensor(
                np.stack(self.rays))  # (N_images, hw, ?)
        self.poses = torch.FloatTensor(self.poses)  # (N_images, 3, 4)
        self.mean_radius = self.poses[:, :3, 3].norm(dim=-1).mean(0).item()
        self.mean_h = self.poses[:, 1, 3].mean().item()

        self.min_radius = self.poses[:, :3, 3].norm(dim=-1).min()
        self.max_radius = self.poses[:, :3, 3].norm(dim=-1).max()
        self.min_theta = np.radians(1) # set to 1~90 for pseudo views
